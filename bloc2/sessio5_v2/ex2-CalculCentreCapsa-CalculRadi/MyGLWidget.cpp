...
void MyGLWidget::paintGL ()
{
  ...
  std::cout << "CENTRE CAPSA: ("<< centre.x<< "," << centre.y << "," << centre.z << ")" <<std::endl;
  calculRadi(minCapsa,maxCapsa);
...
glm::vec3 MyGLWidget::calculCentreCapsa(const glm::vec3 &max,const glm::vec3 &min) {
    ...
    centreCaixa.x = (max.x - min.x)/2;
    centreCaixa.y = (max.y - min.y)/2;
    centreCaixa.z = (max.z - min.z)/2;

    return centreCaixa;
}

float MyGLWidget::calculRadi(const glm::vec3 &centre,const glm::vec3 &max) {
     float x = max.x - centre.x;
     float y = max.y - centre.y;
     float z = max.z - centre.z;

     float radi = (float) sqrt(x*x+y*y+z*z)/2;
