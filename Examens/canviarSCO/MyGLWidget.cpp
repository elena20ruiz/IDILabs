...
MyGLWidget::MyGLWidget (QWidget* parent) : QOpenGLWidget(parent)
{
  ...
  vec_llumAmbient = glm::vec3(1, 1, 1);
...
void MyGLWidget::modelTransformTerra ()
{
  ...
  TG = glm::translate(TG,glm::vec3(1.,-minTerra.y-2.0,0.));
  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);
...
void MyGLWidget::refreshLlumAmbient() {

    glUniform3fv (llumAmbient, 1, &vec_llumAmbient[0]);
}
  ...
  switch (event->key()) {
    ...
    case Qt::Key_R: {
      ...
      case Qt::Key_L: {
           blanc = !blanc;
           if (blanc == 1) {
                focus_ant = vec_posFocus;
                vec_posFocus = glm::vec3(1,1,1);
                refreshPosFocus();

                col_ant = glm::vec3(0,0,0);
                refreshColFocus();
           }
           else {
                vec_posFocus = focus_ant;
                refreshPosFocus();
                vec_colFocus = glm::vec3(0,1,1);
                refreshPosFocus();
           }
          break;
      }
